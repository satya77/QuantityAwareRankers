...
from models.semantic_matchers.colbert.data.loaders import load_collection
from models.semantic_matchers.colbert.infra.run import Run


class Collection:
    def __init__(self, path=None, data=None,data_type=None):
        self.path = path
        self.data = data or self._load_file(path=path,data_type=data_type)
    ...
    def _load_file(self, path, data_type):
        self.path = path
        return self._load_tsv(path,data_type) if path.endswith('.tsv') or path.endswith('.csv') else self._load_jsonl(path)

    def _load_tsv(self, path,data_type):
        return load_collection(path,data_type)
