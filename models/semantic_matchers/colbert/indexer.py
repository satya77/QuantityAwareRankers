...
from models.semantic_matchers.colbert.infra.launcher import Launcher

from models.semantic_matchers.colbert.utils.utils import create_directory, print_message

from models.semantic_matchers.colbert.indexing.collection_indexer import encode


class Indexer:
    def __init__(self, checkpoint,data_type, config=None):
        """
    ...
    def index(self, name, collection, overwrite=False):
        ...
        if index_does_not_exist or overwrite != 'reuse':
            self.__launch(collection,self.data_type)

        return self.index_path

    def __launch(self, collection,data_type):
        manager = mp.Manager()
        ...
        launcher.launch(self.config, collection, shared_lists, shared_queues,data_type)
